package muserver.gameserver.utils;
...
//			log.warn("Buffer must be at index 0!");
		...
		int header = MuCryptUtils.GetHeaderSize(buff);
		int decodedSize = MuCryptUtils.GetDecodedSize(buff);
...
//			log.warn("Buffer must be at index 0!");
		...
		int header = MuCryptUtils.GetHeaderSize(buff);
		int packetSize = MuCryptUtils.GetPacketSize(buff);
		int decodedSize = MuCryptUtils.GetDecodedSize(buff);
...
//			log.warn(PrintData.printData(buff.nioBuffer()));
		...
		for (int i = 0; i < size; i += 11) {
			...
			int Result = BlockDecode(decrypted, MuCryptUtils.getAsUByteArray(encrypted, uByteArray), converter, KeyReader.getDec1KeySet());
			if (Result != -1) {
	...
	private static int BlockDecode(ByteBuf decrypted, short[] InBuf, ByteBuf converter, long[] Keys) {
		...
		MuCryptUtils.writeByteArray(converter, Shift);
		MuCryptUtils.flushArray(Shift, 0, 4);
